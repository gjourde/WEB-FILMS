...
if (isset($_POST['titre']) and isset($_POST['realisateur']) and isset($_POST['affiche']) and isset($_POST['annee'])) {
    ...
    $ajoutFilm = new Films($_POST['idFilm'], $_POST['titre'], $_POST['realisateur'], $_POST['affiche'], $_POST['annee']);
    $status = $filmsDao->add($film);
    if ($status) {
        echo $twig->render('ajouter_film.html.twig', ['status' => "Ajout du Film confirmé ", 'ajoutFilm' => $ajoutFilm]);
    } else {
